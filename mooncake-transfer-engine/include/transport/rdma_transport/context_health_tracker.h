...
// thread's PORT_ACTIVE recovery (set_active(true), then reset()) such that
...
class ContextHealthTracker {
   public:
    ...
    // Full reset: PORT_ACTIVE recovery, or a fatal async event taking
    ...
    // event's context-flag update (set_active(true) for PORT_ACTIVE, false
    // for fatal events) so it cannot interleave with a concurrent submitter
    // trip's flag update.
