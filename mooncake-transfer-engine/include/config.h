...
struct GlobalConfig {
    ...
    // streak must span before the context breaker may trip. A submit batch
    // targets a single peer, so a streak against one dead/restarting peer is a
    // peer problem, not evidence of local RNIC failure, and must never
    // deactivate the context. 1 = legacy single-peer tripping. Range [1, 64].
    // Override via MC_CONTEXT_FAILURE_MIN_PEERS.
